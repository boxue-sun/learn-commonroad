...
class EgoObservation(Observation):
    ...
    def observe(self, ego_lanelet: Lanelet, ego_vehicle: Vehicle, episode_length: int) -> Union[ndarray, Dict]:
        ...
        if self.observe_relative_heading:
            ...
            self.observation_dict["relative_heading"] = np.array([relative_heading])

        if self.observe_global_turn_rate:
            # 检查是否有yaw_rate属性，如果没有则使用默认值
            if hasattr(ego_state, 'yaw_rate'):
                self.observation_dict["global_turn_rate"] = np.array([ego_state.yaw_rate])
            else:
                self.observation_dict["global_turn_rate"] = np.array([0.0])
