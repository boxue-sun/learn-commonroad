...
class HybridReward(Reward):
    ...
    def calc_reward(self, observation_dict: dict, ego_action: Action) -> float:
        ...
        # 角速度惩罚
        if "reward_angular_velocity" in self.reward_configs:
            yaw_rate = getattr(ego_action.vehicle.state, "yaw_rate", 0.0)
            reward += self.reward_configs["reward_angular_velocity"] * abs(yaw_rate)
