...
def main():
    ...
    env_kwargs = {
        ...
        "reward_configs": {
            "hybrid_reward": {
                ...
                "reward_collision": -1000.0,        # 发生碰撞：大负奖励
                "reward_off_road":-800.0,         # 偏离道路：中等负奖励
                "reward_time_out": -300.0,         # 超时：中等负奖励
                
                # ==================== 目标导向奖励 ====================
                "reward_closer_to_goal_long": 40.0,  # 向目标纵向移动：小正奖励
                "reward_closer_to_goal_lat": 30.0,   # 向目标横向移动：小正奖励
                "reward_goal_distance": -15.0,        # 距离目标惩罚：小负奖励
                "reward_goal_angle": -30.0,           # 目标角度偏差惩罚：小负奖励
        ...
        # ==================== 场景数据路径配置 ====================
        "meta_scenario_path": "pickles_scenario_DEU_AAH-4_2002_T-1/meta_scenario",      # 元场景数据路径
        "train_reset_config_path": "pickles_scenario_DEU_AAH-4_2002_T-1/problem_train", # 训练场景数据路径
        "test_reset_config_path": "pickles_scenario_DEU_AAH-4_2002_T-1/problem_test",   # 测试场景数据路径
    ...
    # 初始化DQN智能体
    # ========== 阶段式关键节点：stage one-hot 拼接到状态 ==========
    num_stages = 4  # 0:启动, 1:里程碑1, 2:里程碑2, 3:抵达

    def build_aug_state(obs: np.ndarray, stage: int) -> np.ndarray:#给状态 “加标签”
        one_hot = np.zeros((num_stages,), dtype=np.float32)
        stage_idx = min(max(int(stage), 0), num_stages - 1)#限制阶段索引范围，防止 stage 超出 0~3
        one_hot[stage_idx] = 1.0
        return np.concatenate([obs.astype(np.float32), one_hot], axis=0)

    def goal_distance(obs: np.ndarray) -> float:#算 “到目标的直线距离”
    ...
    def lane_offset(obs: np.ndarray) -> float:#算 “车道偏移程度”
        return float(abs(obs[2])) if len(obs) > 2 else 0.0

    aug_state_dim = env.observation_space.shape[0] + num_stages
    ...
    print(f"开始多场景DQN训练，状态维度(含stage): {aug_state_dim}, 动作维度: {action_dim}")
    print(f"使用场景数量: {len(os.listdir('pickles_multi_scenarios/problem_train'))}")
    print(f"核心机制：显式Q网络 + 目标网络 + 经验回放 + ε-贪婪策略 + 阶段式奖励")
    ...
    for episode in range(total_episodes):
        ...
        state_raw = env.reset()
        # 阶段初始化：基于初始距离自动生成里程碑
        d0 = goal_distance(state_raw)
        th1 = max(50.0, 0.5 * d0)  # 里程碑1：50米或初始距离的一半
        th2 = max(20.0, 0.2 * d0)  # 里程碑2：20米或初始距离的20%
        final_th = 1.0             # 抵达阈值：1米
        stage = 0
        prev_d = d0
        prev_action = None

        # 构建增强状态
        state = build_aug_state(state_raw, stage)
        total_reward = 0
        done = False
        ...
        while not done:
            ...
            # 执行动作，与环境交互
            next_state_raw, env_reward, done, _ = env.step(action)
            curr_d = goal_distance(next_state_raw)

            # 推进与完成逻辑（按阶段）
            stage_progress = max(0.0, prev_d - curr_d)  # 距离减少为正
            stage_transition_bonus = 0.0
            if stage == 0 and curr_d <= th1:
                stage = 1
                stage_transition_bonus +=100.0
            elif stage == 1 and curr_d <= th2:
                stage = 2
                stage_transition_bonus +=150.0
            elif stage == 2 and curr_d <= final_th:
                stage = 3
                stage_transition_bonus += 300.0

            # 安全与稳定性项
            lane_pen = -0.05 * lane_offset(next_state_raw)  # 车道偏移惩罚
            moving_away_pen = -0.1 * max(0.0, curr_d - prev_d)  # 远离目标惩罚
            lat_change_pen = 0.0
            if prev_action is not None:
                lat_change_pen = -0.1 * abs(int(action[1]) - int(prev_action[1]))
            prev_action = action.copy()
            prev_d = curr_d

            # 阶段推进奖励（每步）
            stage_gain = 2.0 * stage_progress
            ...
            shaped_basic = stage_gain + stage_transition_bonus + lane_pen + moving_away_pen + lat_change_pen + terminal_bonus
            # 融合环境奖励
            shaped_reward = 0.2 * env_reward + 0.8 * shaped_basic

            # 将动作转换为单个索引用于存储
            action_idx = action[0] * 5 + action[1]  # 假设lat_steps=5
            # 存储经验到回放池
            next_state = build_aug_state(next_state_raw, stage)
            agent.replay_buffer.add(state, action_idx, shaped_reward, next_state, done)
            # 训练Q网络
            agent.learn()
